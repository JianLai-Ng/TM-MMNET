...
from tpoDCAblock import resnet18_pos_attention
...
def parse_args():
    ...
    parser.add_argument('--batch_size', type=int, default=32, help='Batch size.') #as per biggest loso subject 
...
class MMNet(nn.Module): #ADJUSTED
    def __init__(self, embedding_au_size = None):
        ...
        self.conv_act = nn.Sequential(
            ...
            # nn.BatchNorm2d(180),
            nn.Conv3d(in_channels=3, out_channels=128, kernel_size=(1,3,3), stride=(1,2,2),padding=(0,1,1), bias=False,groups=1),
            nn.BatchNorm3d(128),
            nn.ReLU(inplace=True),
            nn.Conv3d(in_channels=128, out_channels=180, kernel_size=(3,1,1), stride=(1,1,1),padding=(1,0,0), bias=False,groups=1),
            nn.BatchNorm3d(180),
            nn.ReLU(inplace=True),
            )
        ...
        patch_size=1, embed_dim=1024, depth=2, num_heads=4, mlp_ratio=4, qkv_bias=True,norm_layer=partial(nn.LayerNorm, eps=1e-6),drop_path_rate=0.)
    ...
    def forward(self, onset, x1, x2, x3, x4, if_shuffle):
        ...
        # Position Calibration Module (subbranch)
        POS =self.vit_pos(self.resize(onset)).transpose(1,2).view(B,1024,14,14)
...
def run_training(args):
    ...
    #         '20', '21', '22', '15', '6', '25', '7']
    #LOSO_full = list([i for i in range(1,27)])
    
#     if int(args.part)<=0:
#         if int(args.part) == -1:
#             LOSO = ['17', '26']
#         else:
#             LOSO = list([str(i) for i in range(1,27)])[5*( int(args.part) *-1  ):min(5*(  int(args.part)*-1  +1),27)]
#     else:
#         LOSO = [int(args.part),]
    ...
    for subj in LOSO:
        ...
        for i in range(1, int(args.epochs)+1):
            ...
            entry_append = [subj, i, epoch_lr, train_loss.detach().numpy().tolist(), train_acc.detach().numpy().tolist(), running_loss.detach().numpy().tolist(), acc,  AVG_F1.tolist()]
            print(entry_append)
...
# FOR EACH SUBJECT, ADD ONCE POST TRAINING #            
        ...
        testing_metrics_entry = [subj, i, best_f1_epoch, val_dataset.__len__(), str(max_pos_label.tolist()), str(max_pos_pred.tolist()), 
                                 str(max_TP.tolist()), str(best_FP.tolist()), str(best_FN.tolist()), subject_best_f1]
