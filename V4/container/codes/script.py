...
class MMNet(nn.Module): #ADJUSTED
    def __init__(self, embedding_au_size = None):
        ...
        self.conv_act = nn.Sequential(
            ...
            nn.Conv3d(in_channels=128, out_channels=256, kernel_size=(4,1,1), stride=(1,1,1),padding=(0,0,0), bias=False,groups=1),
            nn.BatchNorm3d(256),
            nn.ReLU(inplace=True),
            )
    ...
    def forward(self, onset, x1, x2, x3, x4, if_shuffle):
        ...
        # Position Calibration Module (subbranch)
        POS =self.vit_pos(self.resize(onset)).transpose(1,2).view(B,512,14,14)
            ...
            with torch.no_grad():
                ...
                if AVG_F1 == max_f1:
                    if max_f1_best_acc < acc:
                        ...
                        best_FN = FN
                        best_FP = FP
                        print('NEW Best Val F1:',str(max_f1),', at epoch:',str(best_f1_epoch))
                        #save model
