...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, au_size, num_classes=1000, zero_init_residual=False,
        ...
        self.conv1 = nn.Conv2d(512, self.inplanes, kernel_size=3, stride=1,padding=1,
                               bias=False,groups=1)
        self.bn1 = norm_layer(self.inplanes)
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2,padding=1)
        self.maxpool1 = nn.MaxPool2d(kernel_size=2, stride=2)
        self.layer1 = self._make_layer(block, 512, layers[0],groups=1)
        ...
        self.embedau = nn.Linear(2048* block.expansion*196, au_size)
    ...
    def _forward_impl(self, x,POS, embedding_au_size):##x->input of main branch; POS->position embeddings generated by sub branch
        ...
        pre_embed = torch.flatten(x, 1)


        embed = self.embedau(pre_embed) #embeddings

        return pre_embed, embed

    def forward(self, x,POS, embedding_au_size):
        return self._forward_impl(x,POS, embedding_au_size)


def _resnet(arch, block, layers, pretrained, progress, au_size, **kwargs):
    model = ResNet(block, layers,au_size, **kwargs)
    if pretrained:
...
def resnet18_pos_attention(au_size, pretrained=False, progress=True, **kwargs):
    ...
    """
    return _resnet('resnet18', CABlock, [1, 1, 1, 1], pretrained, progress, au_size = au_size,
                   **kwargs)
